...
/// @title HederaBidEscrow — USDC HTS bid escrow on Hedera testnet.
/// @dev Additive cash rail. No CRE receiver; award is operator-only.
///      Same auction/bid layout as Arc BidEscrow so the UI can reuse getters.
contract HederaBidEscrow is Ownable, ReentrancyGuard {
    ...
    /// @param _usdc     Hedera USDC HTS (EVM long-zero of 0.0.429274).
    ...
    function associateUsdc() external {
        _tryAssociate();
    }
    ...
    function _tryAssociate() internal {
        // HIP-719: no-op on MockUSDC / already-associated accounts.
        (bool ok,) = address(usdc).call(abi.encodeWithSignature("associate()"));
        ok;
    }
