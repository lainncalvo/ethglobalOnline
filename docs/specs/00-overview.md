...
| Buyer A | Arc (bids), Hedera (must be eligible) | Bids 1,450 USDC — loses, withdraws | `BUYER_A` (whitelisted + KYC) |
| Buyer B | Arc, Hedera | Bids 1,520 USDC — wins, receives the bond | `BUYER_B` (whitelisted + KYC) |
...
| 2 | List | Hedera | Seller | `bond.createHoldByPartition(0x…01, Hold{amount, expiration = deadline + 72h + 1h, escrow = ExitAuction, to = 0x0, data = "0x"})` → `holdId` from `HeldByPartition` event → `ExitAuction.createAuction(token, 0x…01, holdId, amount, deadline, reserveCommitment)` → frontend POSTs `{reserve, salt}` to the backend. Bond stays in the seller's wallet and keeps earning coupons. → specs 01, 05 |
| 3 | Mirror | Arc | Operator | `BidEscrow.registerAuction(ref, seller, deadline, reserveCommitment, hederaAuctionId)`. → spec 02 |
| 4 | Bid | Arc | Buyers A, B | `USDC.approve(BidEscrow, amt)` → `BidEscrow.placeBid(ref, amt)`. Buyer C sees "not eligible" and cannot bid from the UI. → specs 02, 05 |
...
Contracts on both chains (L2, L3) share one binding key, `ref`, and one report ABI, so they can be built and tested independently against mocks. The CRE workflow (L4) and the backend (L5) share one pure TypeScript award function (`packages/shared/src/award.ts`), so the local fallback and the confidential path cannot disagree. The frontend (L6) only talks to the two contracts through viem and to the backend through the REST contract in spec 04. Issuance (L1) is a runbook, not code: its only output is a bond address and three eligible wallets. Docs and video (L7) consume everything.
