...
|---|---|---|---|---|---|---|
| 0:00–0:20 | Hook | Title card with the project name and one line; then the architecture PNG for 3 s | — | — | — | "The ECB counted 183 tokenized bonds issued since 2018. Only twenty of them ever traded. Issuance is solved; exit is not. This is an exit auction for bonds issued with Hedera's Asset Tokenization Studio." |
...
| 1:45–2:20 | Confidential award | Terminal: `cre workflow simulate --broadcast` output: TEE banner, secrets fetched, snapshot and screening fetched, "commitment verified", outcome AWARDED; ArcScan `Awarded(source=CRE)` tx | Operator (CRE key) | ArcScan tx of `onReport` | Chainlink | "At the deadline, a Chainlink CRE confidential workflow runs. Inside the enclave it fetches the sealed reserve and a compliance screening with secret keys, checks every bidder, and computes the award. Only the winner and the price leave the enclave, as a report the DON writes to Arc. Without this workflow the escrow cannot award." |
